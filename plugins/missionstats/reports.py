...
class Sorties(report.EmbedElement):
    ...
    def render(self, member: Union[discord.Member, str], period: Optional[str]) -> None:
        sql = "SELECT mission_id, init_type, event, time FROM missionstats WHERE init_id = %s AND event IN " \
        ...
        try:
            with closing(conn.cursor(cursor_factory=psycopg2.extras.DictCursor)) as cursor:
                ...
                df = self.sorties.groupby('plane').agg(count=('time', 'size'), mean_time=('time', 'mean')).reset_index()
                planes = sorties = times = ''
                for index, row in df.iterrows():
                    planes += row['plane'] + '\n'
                    sorties += str(row['count']) + '\n'
                    times += utils.convert_time(row['mean_time'].total_seconds()) + '\n'
                self.embed.add_field(name='Planes', value=planes)
                self.embed.add_field(name='Sorties', value=sorties)
                self.embed.add_field(name='Flighttime', value=times)
