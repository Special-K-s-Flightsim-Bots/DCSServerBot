CREATE TABLE IF NOT EXISTS pu_events (id SERIAL PRIMARY KEY, init_id TEXT NOT NULL, target_id TEXT, server_name TEXT NOT NULL, event TEXT NOT NULL, points DECIMAL NOT NULL, time TIMESTAMP NOT NULL DEFAULT NOW(), decay_run INTEGER NOT NULL DEFAULT -1);
...
CREATE TRIGGER IF NOT EXISTS tgr_pu_events_insert AFTER INSERT ON pu_events FOR EACH ROW EXECUTE PROCEDURE pu_events_insert();
