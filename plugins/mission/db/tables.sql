CREATE TABLE IF NOT EXISTS players (ucid TEXT PRIMARY KEY, discord_id BIGINT NOT NULL DEFAULT -1, name TEXT, manual BOOLEAN DEFAULT FALSE, first_seen TIMESTAMP DEFAULT NOW(), last_seen TIMESTAMP, watchlist BOOLEAN NOT NULL DEFAULT FALSE, vip BOOLEAN NOT NULL DEFAULT FALSE);
CREATE INDEX IF NOT EXISTS idx_players_discord_id ON players(discord_id);
CREATE TABLE IF NOT EXISTS bans (ucid TEXT PRIMARY KEY, banned_by TEXT NOT NULL, reason TEXT, banned_at TIMESTAMP NOT NULL DEFAULT NOW(), banned_until TIMESTAMP NOT NULL DEFAULT TO_DATE('99991231','YYYYMMDD'));
