...
from core import Status, Server, ServerImpl, Autoexec, utils, InstanceImpl, ServerMaintenanceManager, proxy, Node
...
class MonitoringService(Service):
    ...
    async def _load_baselines_from_db(self) -> None:
        ...
        """
        if not self.get_config().get('ddos_detect', False):
            return
    ...
    async def _ddos_block(self, server_name: str, port: int, proto: str,
        ...
        """
        Create per-instance firewall rules to restrict DCS access to known players
        plus any configured whitelist IPs.
        Called when DDoS action is 'block'.
        """
        rule_name = f"DCS-block-{proto.upper()}-{port}"
        ...
        try:
            response = await self._send_helper_command(
                f"restrict {rule_name} {proto} {port} {ip_str}"
            )
            if response.startswith('OK'):
                self.log.info(f"DDoS block: {response[3:]}")
            else:
                self.log.warning(f"DDoS block failed: {response}")
        except Exception as ex:
            self.log.warning(f"DDoS block command failed: {ex}")



    async def _ddos_unblock(self, server_name: str, port: int, proto: str) -> None:
        """
        Remove per-instance firewall rules, restoring global rule effect.
        """
        rule_name = f"DCS-block-{proto.upper()}-{port}"

        try:
            response = await self._send_helper_command(f"restore {rule_name}")
            if response.startswith('OK'):
                self.log.info(f"DDoS unblock: {response[3:]}")
            else:
                self.log.warning(f"DDoS unblock failed: {response}")
        except Exception as ex:
            self.log.warning(f"DDoS unblock command failed: {ex}")
    ...
    async def _refresh_block(self, server_name: str, port: int, proto: str) -> None:
        ...
        try:
            response = await self._send_helper_command(
                f"restrict {rule_name} {proto} {port} {','.join(ips)}"
            )
            if response.startswith('OK'):
    ...
    async def start(self):
        ...
        ddos_cfg = self.get_config().get('thresholds', {}).get('DDoS', {})
        if ddos_cfg.get('action') == 'block':
    ...
    async def ddos_whitelist(self, node: Node, ip: str) -> str:
        ...
        """
        config = self.get_config()
        ddos_cfg = config.setdefault('thresholds', {}).setdefault('DDoS', {})
    ...
    async def ddos_unwhitelist(self, node: Node, ip: str) -> str:
        ...
        """
        config = self.get_config()
        ddos_cfg = config.get('thresholds', {}).get('DDoS', {})
    ...
    async def monitoring(self):
        try:
            ...
            if self.get_config().get('ddos_detect', False):
