...
INSERT INTO version (version) VALUES ('v3.9') ON CONFLICT (version) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS nodes (guild_id BIGINT NOT NULL, node TEXT NOT NULL, last_seen TIMESTAMP DEFAULT now() AT TIME ZONE 'utc', PRIMARY KEY (guild_id, node));
CREATE TABLE IF NOT EXISTS instances (node TEXT NOT NULL, instance TEXT NOT NULL, port BIGINT NOT NULL, server_name TEXT, last_seen TIMESTAMP DEFAULT now() AT TIME ZONE 'utc', PRIMARY KEY(node, instance));
...
CREATE TABLE IF NOT EXISTS intercom (id SERIAL PRIMARY KEY, node TEXT NOT NULL, time TIMESTAMP NOT NULL DEFAULT now() AT TIME ZONE 'utc', data JSON, priority INTEGER NOT NULL DEFAULT 0);
CREATE INDEX IF NOT EXISTS idx_intercom_node ON intercom (node);
CREATE TABLE IF NOT EXISTS files (id SERIAL PRIMARY KEY, name TEXT NOT NULL, data BYTEA NOT NULL, created TIMESTAMP NOT NULL DEFAULT now() AT TIME ZONE 'utc');
